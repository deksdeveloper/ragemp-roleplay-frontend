...
mp.events.add("LoadXMR", (stations) => {
	...
	if(!mp.browsers.exists(xmrCEF)) {
		xmrCEF = mp.browsers.new("package://files/xmr-sistem/cef/xmr.html");
	}
...
mp.events.add("SetStation", (stations) => {
	...
	stations = JSON.parse(stations);
	if(!stations) {
		mp.gui.chat.push("İstasyonları alırken bir hata oluştu.");
	} else {
		let categories = [];
		for(let i = 0; i < stations.length; i++) {
		...
		let ui = new Menu("XMR İstasyonu Ayarla", "Bir tür seçin", new Point(50, screenRes.y * 0.3));
		let turnoff = new UIMenuItem("Kapat", "Radyo'yu kapat.");
		ui.AddItem(turnoff);
		
		for(let i = 0; i < categories.length; i++) {
			let btn = new UIMenuItem(categories[i], "Bu türü görüntüle");
			ui.AddItem(btn);
		}
		
		let cancel = new UIMenuItem("İptal", "Bu menüyü kapat.");
		...
		ui.ItemSelect.on((item) => {
			if(item == turnoff) {
				...
				ui.Close();
				let selection = new Menu("XMR İstasyonu Ayarla", "Bir tür seçin", new Point(50, screenRes.y * 0.3));
				for(let j = 0; j < stations.length; j++) {
					if(stations[j].Genre == item.Text) {
						let sel = new UIMenuItem(stations[j].Name, "Bu türü görüntüle");
						selection.AddItem(sel);
					}
				}
				let back = new UIMenuItem("Geri", "Türlere geri dön");
				...
				selection.ItemSelect.on((itm) => {
					if(itm == back) {
						...
						mp.gui.chat.push("Seçilen Radyo İstasyonu", itm.Text);
					}
				});
			}
		})
