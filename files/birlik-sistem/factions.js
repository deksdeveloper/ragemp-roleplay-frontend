...
function populateFactionsList(inventoryJson ,leader, member, invite) {
		...
		for(let i = 0; i < factions.length; i++) {
			...
			optionContainer.textContent = 'KATIL';
			...
			if(member == 0)
			{
				...
				if(invite > 0)
				{
					if(invite == item.id)
					{
						optionContainer.textContent = 'AYRIL';
...
function GetFactionTypeName(tip)
{
	var type = "Belirsiz";	
	if(tip == 1){ type = "LSPD";}
	else if(tip == 2){ type = "LSMD";}
	else if(tip == 3){ type = "San News";}
	else if(tip == 5){ type = "Mafya";}	
	else if(tip == 4){ type = "Çete";}
	else if(tip == 7){ type = "Government";}
	else if(tip == 6){ type = "Mekanik";}
	else { type = "Belirsiz";}
	return type;
}
...
function populateFactionMenu(factionInfo,requestsList ,membersList, ranksList) {
		...
		for(let i = 0; i < members.length; i++) {
			...
			col1.textContent = item.name;
			col2.innerHTML  = '<span  id="member' + item.character + '">'+ ranks[item.rank].name + '</span>  <button class="btn btn-sm btn-info" onclick="editMember(' + item.character + ',' + (item.rank+1) + ')" data-toggle="modal" data-target="#RankModal" >DÜZENLE</button>';
